...
- [x] Web 端已完成固定 CSV/TXT 的模板下载、粘贴/上传、行编辑、Preview、Apply 和结果查看闭环。
- [ ] 继续接入 Excel 文件解析、错误筛选/下载和大批量任务进度展示。
...
> 状态：部分完成。当前 Web 端先完成固定 CSV/TXT 结构化行闭环，Excel 解析、错误明细下载和大批量进度查询继续在后续迭代实现。
...
- [x] 提供固定 CSV 模板下载。
- [x] 展示字段说明、授权方案编码和部门编码填写说明。
- [x] 当前限制为 CSV/TXT，上传后展示文件名、行数和解析结果。
- [ ] 接入 Excel 解析和文件大小限制。
