...
- [x] 完整页面按“基本信息 → 授权方案 → 角色授权”步骤条分开展示，避免表单和授权配置连续纵向堆叠。
- [x] 增加管理员用户详情接口 `GET /api/user/{uid}`，支持编辑页刷新和直接访问。
- [x] 创建用户后返回用户 ID，并在同一开通流程中继续完成角色授权。
- [x] 用户新增/编辑改为“基本信息 → 授权方案 → 角色授权 → 最后提交”，步骤切换不提前持久化，后端一次性事务提交用户资料、多角色授权和移除列表；第 03 步按 `3.1`、`3.2` 子节点切换角色，角色增删统一在第 02 步完成。
...
- [x] 授权方案步骤支持套用多 Role 授权方案；已有用户角色自动加载，重复角色跳过并提示，第 02 步支持角色移除，角色授权步骤通过 `3.x` 子节点逐个调整 Boundary。
- [x] 提供授权方案列表、独立创建/修改页面、启用、停用和删除操作，并复用访问控制菜单入口。
- [x] 授权方案管理和单用户套用已完成；批量导入后端的结构化行契约、任务、校验和 Preview/Apply 已完成。
- [x] Web 端已完成只包含用户基本信息的中文 Excel 模板下载、CSV/TXT/Excel 上传、行编辑、固定部门/语言/时区/授权方案选择、Preview、Apply、错误处理和结果查看闭环。
- [x] 大批量 Apply 已改为有界后台任务，Web 端轮询任务详情展示处理进度和最终错误明细。

## 问题背景

当前新增用户和角色授权曾经是两个相互独立的操作：

1. 管理员先填写并保存用户基本信息。
2. 用户资料接口与授权接口分开调用，存在中间持久化和失败后留下半成品用户的风险。
3. 管理员需要在同一页面继续操作 RoleAssignment，流程边界不清晰。
...
按需展开高级 Permission 配置
  ↓
提交（后端事务内 Preview/Apply）
  ↓
完成
```

基本信息和授权配置在最后提交前只保存在页面状态中；用户取消或刷新不会创建半成品用户。已有用户仍可能因为历史数据或其他流程处于“未配置授权”状态，列表必须继续明确展示该状态。
...
- 确认用户新增/编辑与多角色 RoleAssignment 保留、修改、移除一次性提交的接口契约。
- 确认用户没有 RoleAssignment 时的授权状态定义。
- 确认步骤切换不产生写请求，最终提交失败时用户资料和授权一起回滚。
...
- 新增 `UserOnboardingFrom` 和 `UserOnboardingVO`，同时承载用户资料与多角色 RoleAssignment 配置、移除列表。
- 新增 `POST /api/user/onboarding` 和 `PUT /api/user/onboarding`，分别处理新增和编辑。
- 后端在同一事务内完成用户资料、身份标识、密码凭证、审计、多角色 RoleAssignment 变更和撤销；任一环节失败全部回滚。
- 前端 API 使用具体的 `UserOnboardingVO` 响应类型，不再调用旧的分步创建/更新接口。

**接口方向**：

```text
POST /api/user/onboarding
response: { id, real_name }
```

#### 2.2 将用户新增/编辑改为流式两步向导

**操作**：

- 第一步只校验并暂存用户基本信息，主按钮为“下一步”。
- 第二步配置角色和 Boundary，主按钮为“提交”。
- 最后一次提交前不创建用户、不修改用户资料、不写入 RoleAssignment。
- 用户编辑时保留“编辑信息”和“配置授权”两个明确入口，但二者都使用同一套最终提交接口。
- 角色授权编辑器只负责加载字典、编辑本地状态、校验并生成请求，不直接调用 Preview/Apply。
...
- 单用户流程已支持选择多 Role 方案并通过各 RoleAssignment Preview/Apply；编辑已有用户支持移除角色，批量导入继续复用多角色授权链路。
...
```text
用户信息 Sheet（Excel/CSV 模板）
  real_name
  phone
  email

工号由后端在 Preview 阶段自动生成并保存，管理员不需要在文件中填写。

本次导入固定配置（在 Web 数据预览上方选择）
  department_code
  language
  timezone
...
- [x] 提供只包含用户基本信息的中文 Excel 模板下载。
- [x] 在数据预览上方统一选择部门、语言、时区和授权方案，并说明固定配置会应用到本次导入的全部用户；错误明细导出使用中文表头，访问边界和授权边界使用中文展示。
...
- 新增/编辑用户点击“下一步”只进行前端校验，不发送写请求。
- 最后一页点击“提交”只发送一次用户资料与授权请求，并在成功后返回列表。
