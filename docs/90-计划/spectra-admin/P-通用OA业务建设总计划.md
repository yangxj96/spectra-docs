...
| 消息 | Core 已有 `sys_notification` 与消息设置基础，后续按统一通知计划提取为独立模块 | 待办、审批结果、公告、日程和到期提醒 |
...
真实闭环验收（使用 `spectra-admin/.mise.local.toml` 的数据库/Redis 配置；根目录同名文件仅是维护者可选覆盖）：
...
- [[90-计划/spectra-admin/P-统一通知模块建设计划]] — 站内信、短信、邮件、模板和可靠投递的跨业务基础能力。
