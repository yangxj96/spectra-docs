...
**P0 阶段 0、阶段 1 和阶段 2 已完成首版；阶段 3（报销、采购与资产首版纵切）进行中，其余阶段待启动**

本轮首版覆盖统一申请内核、请假申请后端闭环、公告发布与阅读回执、个人日程、会议冲突/参会响应、工作台聚合、费用报销、采购申请和资产管理首版纵切、Web/移动端高频入口以及建表 SQL 和知识库同步。定时发布后台调度、公告附件、完整角色化卡片、消息深链、报销财务增强、办公用品库存和集成/E2E 测试仍列为后续补齐项。

> 本计划是 OA 建设的总控计划。原 [[90-计划/spectra-admin/P-OA模块对标分析与开发计划]] 保留为历史现状分析，不再作为实施基线。
...
报销、采购、资产、文档、合同等放在后续阶段，避免首期横向铺开后没有任何模块真正可用。当前已在阶段 3 完成报销和采购首版纵切，作为行政与费用闭环的两个可用业务基线。
...
| `asset` | 仅部门字段与分页查询 | 重建为资产台账和领用生命周期 |
...
当前首版先落地固定资产纵切：`oa_asset` 台账、`oa_asset_category` 分类和 `oa_asset_operation` 生命周期记录，支持手工建账、采购收货生成草稿、领用、归还、调拨、维修、报废及操作履历查询。办公用品 SKU/库存、盘点、二维码和资产审批流作为后续增强。

### 4.9 费用报销（P1）

#### 当前实施进度：报销首版纵切已完成

首版已覆盖报销草稿、费用明细、凭证附件关联、明细合计校验、提交审批、撤回/取消、审批回调和付款登记。后续继续补充财务审核多级路由、预算校验、发票查重/OCR、付款失败重试和统计导出。
...
- [ ] 文档目录、版本、ACL、发布审批、预览下载和审计。
- [ ] 制度发布后通过公告/消息通知目标用户。
- [ ] 合同台账、相对方、版本、审批、签署、履约节点和到期提醒。
- [ ] 下载鉴权、敏感字段脱敏和水印策略预留。
- [ ] 文档/合同检索与归档。

验收：

- [ ] 无文档权限的用户即使知道文件 ID 也无法预览或下载。
- [ ] 文档新版本不覆盖历史文件，可查看版本链并回滚。
- [ ] 合同节点到期提醒只发送一次且可追踪处理状态。
...
| P1 | 阶段 3：行政与费用 | 进行中：报销、采购、资产首版已完成，办公用品库存和财务增强待实施 | 阶段 1，可与阶段 2 后半并行 |
| P1-P2 | 阶段 4：文档与合同 | 待启动 | Upload、Notification 完整可用 |
| P2 | 阶段 5：通用化与运营 | 待启动 | 前述业务已有稳定数据 |
...
- [[90-计划/spectra-admin/P-OA模块对标分析与开发计划]] — 历史现状分析，已被本计划整合。
- [[90-计划/spectra-admin/P-工作流模块完整实现计划]] — 工作流平台总控计划。
- [[90-计划/spectra-admin/P-Workflow审批流完善计划]] — OA 审批前置能力的历史实施计划。
- [[90-计划/spectra-admin/P-消息中心后端实现计划]] — OA 通知与提醒依赖。
