...
**P0 阶段 0、阶段 1 已完成首版；阶段 2 及后续阶段待启动**

本轮首版覆盖统一申请内核、请假申请后端闭环、基础工作台聚合、Web/移动端高频入口以及建表 SQL 和知识库同步。通用申请中心的完整管理端、附件/抄送操作页、移动端审批深链、个人日程忙闲联动和集成/E2E 测试仍列为后续补齐项，因此阶段任务清单中的未完成项保持未勾选。
...
报销、采购、资产、文档、合同等放在后续阶段，避免首期横向铺开后没有任何模块真正可用。
...
| P0-P1 | 阶段 2：协同办公 MVP | 待启动 | 阶段 1 |
| P1 | 阶段 3：行政与费用 | 待启动 | 阶段 1，可与阶段 2 后半并行 |
