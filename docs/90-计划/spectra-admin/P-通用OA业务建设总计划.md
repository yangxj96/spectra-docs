...
**P0 阶段 0、阶段 1 和阶段 2 已完成首版；阶段 3（报销纵切）进行中，其余阶段待启动**

本轮首版覆盖统一申请内核、请假申请后端闭环、公告发布与阅读回执、个人日程、会议冲突/参会响应、工作台聚合、费用报销首版纵切、Web/移动端高频入口以及建表 SQL 和知识库同步。定时发布后台调度、公告附件、完整角色化卡片、消息深链、报销财务增强和集成/E2E 测试仍列为后续补齐项，因此阶段任务清单中的未完成项保持未勾选。
...
报销、采购、资产、文档、合同等放在后续阶段，避免首期横向铺开后没有任何模块真正可用。当前已在阶段 3 先完成报销纵切，作为行政与费用闭环的首个可用业务。
...
| P1 | 阶段 3：行政与费用 | 进行中：报销首版已完成，采购/资产待实施 | 阶段 1，可与阶段 2 后半并行 |
