...
> 创建时间：2026-08-11
> 最近调整：2026-08-23（确认统一通知模块 V1 完成；模板管理、受控发送、完整运维页面和真实 SMS/EMAIL Provider 转入后续端到端计划）
> 适用范围：`spectra-admin`，由 Security、Core、OA、Workflow、AI 和后续业务模块共同使用。
 > 关联文档：[[../../10-后端/20-用户与权限]]、[[../../10-后端/75-统一通知模块]]、[[../../20-前端/10-spectra-ui]]、[[../../10-后端/30-系统管理]]、[[90-计划/P-通知中心运维管理与外部渠道接入计划]]。Security 只负责验证码生命周期和认证语义，通知模块负责消息中心与可靠投递；后续通知中心产品闭环由新计划统一推进。
...
- `docs/20-前端/10-spectra-ui.md`：消息中心铃铛、抽屉、完整页面和静态认证路由。
