...
- [~] Phase 9 已开始认证身份运行时迁移：SMS/Email provider 不再读取旧 Account 表，绑定/解绑写入目标 authentication_identity，SecurityUserHelper 删除旧 Account/DataScope 构造路径；SecurityContextAccessor 与 Session 查询/撤销窄端口已接入，core/notification/AI/workflow 的 SecUtil 业务调用已清零；旧模型删除、其他模块旧调用迁移、旧 Redis/表清理和最终端到端验收仍待完成。
