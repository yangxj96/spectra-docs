...
调度管理公共 URL 为 `/api/scheduler/quartz`。`ROLE_ADMIN_SYSTEM` 管理普通 Job，`ROLE_DEV_OPS` 负责立即触发和内置 Job 高风险操作，`ROLE_AUDIT` 只读；Quartz/PostgreSQL 不可用返回 `503 SCHEDULER_DATABASE_UNAVAILABLE`。完整路径见 [[../后端/10-后端模块/07-调度内核]]。
