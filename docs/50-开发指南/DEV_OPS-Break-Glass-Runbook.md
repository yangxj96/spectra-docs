...
> 状态：开发阶段暂缓；确定 `1.0.0` 发布版本后，需由运维负责人完成评审、凭据分权落地和首次隔离演练后生效。本文不保存任何真实密码、TOTP Secret、Recovery Code、私钥或数据库凭据。

Break-glass 是生产环境在正常 Root、MFA 或认证链路不可用时使用的紧急恢复 Root，不是日常管理员账号，也不是开发阶段绕过 MFA 的临时开关。当前没有正式生产环境，因此暂不要求完成凭据保管和演练。
