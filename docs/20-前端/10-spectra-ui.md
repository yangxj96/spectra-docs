...
- 用户批量导入使用完整页面路由 `/system/user/import`，挂在用户管理菜单下；页面当前支持固定 CSV/TXT 模板下载、粘贴/上传、行编辑、后端 Preview、Apply 和结果查看，API 封装位于 `src/api/user/user-import-api.ts`，CSV 解析与摘要工具位于 `src/utils/user-import.ts`。Excel 解析、错误明细下载和大批量进度查询暂未接入。
