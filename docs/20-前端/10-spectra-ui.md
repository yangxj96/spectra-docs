...
- 用户批量导入使用完整页面路由 `/system/user/import`，挂在用户管理菜单下；页面采用与用户编辑一致的左侧编号步骤、中间滚动内容、右侧提示和底部固定操作栏，支持下载带中文表头和下拉校验的 Excel 模板、CSV/TXT/Excel 上传、选择文件后自动解析、行编辑、后端 Preview、Apply、错误分类筛选、带中文表头的错误明细下载和结果查看。部门编码和授权方案编码从当前系统数据生成 Excel 下拉选项，下拉项显示“名称｜编码”，解析后仍映射为接口字段，并兼容旧模板和手工输入的组合值。Apply 返回后台任务后，页面轮询任务详情展示已处理行数和进度，完成后加载错误明细；API 封装位于 `src/api/user/user-import-api.ts`，文件解析与摘要工具位于 `src/utils/user-import.ts`。
