...
消息中心通过 `/notification`、`src/components/NotificationBell/` 和 `src/views/Notification/` 提供未读数、筛选、已读、批量删除、详情和用途×渠道偏好；模板管理通过 `/devops/notification/template` 和 `src/views/Devops/Notification/Template/` 接入真实模板 API，按逻辑模板组聚合展示多个渠道，支持按渠道草稿编辑、预览、保存草稿、保存且发布、发布版本禁用/启用、归档、版本历史和两个版本并列对比。保存、预览和发布前会校验声明变量、引用变量、非法占位符、用途/渠道兼容以及 HTML/URL 安全；验证码用途在编辑器中只显示短信/邮件渠道，后端规则仍是最终门禁。通知运行概览通过 `/devops/notification/overview` 接入真实概览 API，支持窗口选择、自动刷新、渠道状态、队列/失败/UNKNOWN 指标、投递趋势和脱敏错误；Request、Task、Delivery 运维页分别接入真实列表、详情、供应商投递记录、分页筛选、重试/取消、供应商回执和脱敏错误详情。渠道配置通过 `/devops/notification/provider` 接入真实 Provider 配置、健康检查和测试发送 API，支持 `IN_APP`、`SMS`、`EMAIL` 状态、非敏感参数、Secret 覆盖/清除、启停、健康门禁和 `SEND_TEST` 确认后的明确测试地址发送；Secret 和测试地址不回显，未配置或未健康时明确阻断。受控发送通过 `/devops/notification/send` 接入 Preview/Apply API，支持已发布模板版本、用户/部门下级/角色受众、非敏感参数、渠道状态、跳过原因、脱敏样例和 Apply 后跳转 Request。普通管理查询限制最近 31 天，精确 Request/Task 关联查询不受该窗口影响；供应商回执、SMS/EMAIL 失败/UNKNOWN 人工处置、真实浏览器验收和 Provider 沙箱验收均有对应页面与接口验证，通知运维页面全部使用真实接口，不保留 Placeholder 页面。
...
│   │   │   ├── Monitor/
│   │   │   ├── Notification/   # 通知运行概览、Request/Task/Delivery、模板管理、受控发送、渠道配置等通知运维页面
...
`DevopsApplicationHealth` 位于 `/devops/monitor/application-health`，复用 `ServiceMonitorApi.getOverview()` 获取 `/api/service/monitor/overview` 的健康聚合结果，不单独创建重复的依赖连接检查。页面聚焦应用是否具备正常提供服务的条件：顶部展示应用总体状态和监控数据新鲜度；“应用存活状态”展示健康组件；“依赖就绪状态”展示 PostgreSQL、Redis、Quartz、Flowable、对象存储和通知等已注册依赖；“应用运行信息”展示服务名称、主机、操作系统、运行时长和检查耗时。

页面支持 5、10、30 秒自动刷新和手动刷新，刷新失败时保留上一份成功数据，并在卸载时清理轮询定时器。所有 `HEALTHY`、`DEGRADED`、`DOWN`、`UNKNOWN` 等状态和数据新鲜度均转换为中文；页面只显示脱敏说明，不展示连接串、凭据、Session、Token、验证码或防重放随机数原文，安全数据维护继续使用缓存清理页面的安全运行态。
