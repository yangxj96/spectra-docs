...
开发环境按目标模型初始化，不以开发库中的账号或业务数据推断生产迁移结果。包含历史 user-level Scope 或业务数据的数据库，必须按本文执行逐条盘点和人工映射门禁。
...
- 没有任何迁移来源 Scope 被自动转换为 `GrantablePermission` 或 Grant Boundary。

## 只读盘点查询

以下查询只返回聚合或内部 UUID。执行前先确认数据库已完成对应 Flyway 迁移；迁移来源表不存在时只记录“无来源表”，不要为了查询创建兼容表。
...
迁移门禁：存在未映射、重复映射、无 Permission、无 Assignment、规则无法编译或超出 Root/权限边界的记录时，停止数据切换。当前 V1 基线不包含迁移来源的旧运行时结构，也不负责猜测业务权限。
