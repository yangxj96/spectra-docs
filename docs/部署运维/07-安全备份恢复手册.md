# 安全数据库备份、恢复、全局登出与回滚手册

> 本文只描述流程和证据要求，不包含生产连接串、密码、密钥或真实备份位置。执行必须由数据库负责人和 Security/Audit Owner 共同授权。

> 发布门禁：`1.0.0` 发布前执行真实备份恢复、Flyway 回滚和隔离恢复演练；代码、迁移和故障 fail-closed 验证不能替代这些生产运维门禁。

## 适用范围

适用于数据库结构切换、Flyway 升级、权限大批量变更、Session 故障和安全事件。统一审计表追加记录由应用运行账号写入；分区和备份恢复使用独立数据库运维权限。

## 变更前门禁

1. 记录变更单、事件编号、目标 Flyway 版本、回滚负责人和独立复核人。
2. 对 `spectra_core`、`spectra_security`、`spectra_notification`、`spectra_oa` 和 `spectra_workflow` 做一致性快照或可恢复备份。
3. 备份完成后执行独立校验，并在隔离数据库恢复至少一次；只验证“备份文件存在”不算通过。
4. 记录当前 `root_policy`、有效 DEV_OPS 数量、活动 Assignment、Session policy 和 Redis key namespace 统计。
5. 明确全局 logout 的执行窗口。Security Redis 不可用时不得宣称已完成全局 logout，也不得用本地缓存替代撤销。

## 恢复验收

恢复到隔离 PostgreSQL 后至少验证：

- Flyway history 连续且校验通过；
- `spectra_core.sys_audit_event` 及其分区边界与默认分区状态正确，应用运行账号仍只能查询和追加；
- `root_policy` 和有效 DEV_OPS 数量满足最小 Root 约束；
- RoleAssignment、Permission Boundary、AuthenticationIdentity 和 PasswordCredential 数量与备份报告一致；
- 恢复后的审计记录数量、时间范围、主表/分区结构与备份清单核对一致；
- Redis 使用 `sec:*` namespace，抽样 Token Digest 不会在日志或响应中出现；
- 使用测试账户完成登录、刷新、logout、全局 revoke 后，旧 Access/Refresh Token 均被拒绝。

## 全局登出步骤

1. 先写入 `SECURITY_GLOBAL_LOGOUT_STARTED` 审计事件；Audit 不可用则停止。
2. 通过 `SecuritySessionRevocationPort` 按用户、Client 或全量撤销；禁止直接删除任意业务 Redis key。
3. 验证 `sec:session:*`、Token family 和 replay revoke 结果，并保留数量证据；不记录 token 原文。
4. 追加 `SECURITY_GLOBAL_LOGOUT_SUCCEEDED` 或 `SECURITY_GLOBAL_LOGOUT_FAILED`。
5. 让 Web 重新登录并验证 Refresh rotation；旧 Refresh Token 必须被拒绝。

## 回滚规则

- 数据库迁移不使用 `baselineOnMigrate`，不使用 Flyway clean 作为回滚。
- 发现结构或权限异常时，先停止写流量、冻结高风险安全写，再从已验证快照恢复到隔离环境确认方案。
- 任何回滚都必须评估恢复点之后新增的审计事实；不得将旧快照直接覆盖到生产并丢弃这些记录。由 DBA 按变更单使用受控备份或数据库导出核对并补齐缺失事实。
- 回滚后重新执行 Root、统一审计表 append-only 与分区路由、Redis fail-closed、Cookie/CSRF 和旧 Token 失效验收。
- 没有经过隔离恢复验证、未确认数据时间点和责任人的方案，不得在生产执行。

## 证据清单

变更单、备份摘要、恢复日志、Flyway history、Root/Assignment 计数、Audit append-only 结果、Redis revoke 计数、登录/刷新/logout 测试结果、告警编号和复盘结论必须关联事件编号保存。证据中不得包含密码、Token 或私钥。
