...
Break-glass 是 `ROLE_DEV_OPS` 的灾难恢复流程，不是日常管理员入口或普通管理员的替代 API。Root 策略允许多个 DEV_OPS，`maxDevOpsUsers` 默认 3，推荐配置为：
...
- 应用不强制双人审批；生产运维采用独立人员复核和离线凭据分权，审批由运维流程负责。
...
- Normal DEV_OPS 已接管运维，Incident Commander 明确关闭事件。
