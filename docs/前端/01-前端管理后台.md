...
本页保留管理后台的项目级说明和业务能力总览，基础设施专题已经拆分到以下文档，新增代码时应优先在对应专题维护：

| 专题 | 文档 |
|---|---|
| 启动、布局、路由和动态菜单 | [[03-前端启动与路由]] |
| Pinia Store 和状态持久化 | [[04-前端状态管理]] |
| HTTP、Token、CSRF 和接口加解密 | [[05-前端请求与安全通信]] |
| 路由页面、API 和业务模块矩阵 | [[06-前端页面与业务模块]] |
| 公共组件、权限指令和文件上传 | [[07-前端公共组件与文件能力]] |
| 本地开发、测试、构建和联调 | [[08-前端开发测试与构建]] |

页面与请求的最终事实仍以 `spectra-ui/src/` 当前源码为准；本页中的业务说明用于帮助理解模块边界，不替代后端接口和数据模型文档。
...
`src/views/OA/Approval/index.vue`（`/oa/approval`）提供待办/已办、业务详情、同意和驳回；审批中心提供 `/oa/approval/finance/reimbursement`、`/oa/approval/asset/purchase` 和 `/oa/approval/hr/leave` 三个流程类型页面，统一通过 `process_definition_key` 过滤 Workflow 任务。`src/views/OA/Contact/index.vue` 提供基于 Core 用户/部门的组织通讯录。请假产生的考勤影响通过请假详情和后端 `oa_attendance_record` 管理，不单独提供考勤页面。`/oa/notice` 支持发布/撤回/已读，`/oa/calendar` 支持日程创建和删除，`/oa/meeting` 支持会议创建、邀请响应和签到。
...
├── Login/                  # 登录页
├── Dashboard/              # 仪表盘
├── Profile/                # 个人中心（基本资料、密码、安全设置、安全上下文和通知设置）
│   ├── index.vue
│   └── components/
...
同一页面提供“告警”和“运行时诊断”页签：告警页查询摘要、活动事件和规则配置，规则更新使用后端版本号；诊断页展示内存池、GC、线程状态、连接池、Redis 延迟和慢接口，并通过异步任务轮询线程/堆转储状态，只允许下载成功且未过期的文件。刷新和诊断请求均使用局部 loading，不阻塞总览自动刷新。
...
仓库只提交 `.env.example`。本地开发将其复制为 `.env.development`；开发环境的 `VITE_API_URL=https://127.0.0.1:4004/` 直接连接后端 4004。Vite 通过 `SSL_PASSWORD` 加载后端同一份 `files/ssl/keystore.p12`，因此 Web 使用 `https://localhost:5173` 访问，浏览器才能正常携带 Secure Cookie 和读取 CSRF Cookie。需要更换后端端口或连接远程后端时，仅修改本机环境文件。
