...
审批中心复用 Workflow 待办/已办接口，通过可选 `process_definition_key` 按流程类型筛选。已迁移的 `spectra_core` 数据基线包含“审批中心 → 综合审批/财务相关/资产相关/行政人事 → 全部审批/费用报销审批/采购申请审批/请假审批”三级树及内置角色的叶子菜单授权；Web 路由名为 `OAApproval`、`OAApprovalReimbursement`、`OAApprovalPurchase`、`OAApprovalLeave`。
