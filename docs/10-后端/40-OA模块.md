...
> 办公自动化（Office Automation），路径：`spectra-oa`。当前保留 9 个既有基础子模块，并新增通用申请内核、请假与工作台摘要。
...
不新增 `/oa/workbench` 前端页面，复用现有 Dashboard；后端提供 `GET /oa/workbench/summary`，聚合待办数、未读消息数和个人申请状态计数。
