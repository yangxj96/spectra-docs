...
> 办公自动化（Office Automation），路径：`spectra-oa`。当前保留可形成业务价值或闭环的通用申请、请假、审批、协同办公、报销、采购、资产、办公用品、文档、合同和部门统计能力；已移除无数据、无真实业务行为的旧考勤、通讯录和通用报表脚手架。本页按子模块记录实体、API、业务闭环和当前边界，源码与 SQL 是实现事实来源。
