...
回调实现必须按业务状态做幂等保护。当前请假流程注册 KEY 为 `oa_leave_approval`：通过回写 `oa_attendance_record`，驳回/撤回/取消释放预占额度，并向申请人写入 OA 消息。费用报销流程注册 KEY 为 `oa_reimbursement_approval`：审批回写申请状态，付款状态独立保持 `PENDING`，登记付款后转为 `PAID`。
