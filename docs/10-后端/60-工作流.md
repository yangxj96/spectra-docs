...
Web 与移动端均提供审批中心，直接复用 `/workflow/tasks/todo`、`/workflow/tasks/done` 和任务动作端点，不另建 OA 审批任务表。待办/已办支持可选查询参数 `process_definition_key`，用于审批中心按请假、报销或采购流程类型筛选；省略参数时返回当前用户全部任务。2026-08-08 真实接口验收覆盖请假同意/撤回取消、报销驳回取消/同意付款、采购同意执行收货；所有流程进入历史结束态且运行中任务归零。
