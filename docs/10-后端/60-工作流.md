...
任务查询 VO 返回 `processDefinitionKey`、`businessKey`，供 Web 审批中心定位 OA 业务详情。审批、驳回、转办和委派等写操作均从认证上下文取得当前用户名，并校验任务办理人所有权，不能只凭任务 ID 越权办理。OA 详情可见范围同时包含申请人、同部门用户、当前办理人和历史办理人。
...
Web 审批中心直接复用 `/workflow/tasks/todo`、`/workflow/tasks/done` 和任务动作端点，不另建 OA 审批任务表。待办/已办支持可选查询参数 `process_definition_key`，用于审批中心按请假、报销或采购流程类型筛选；省略参数时返回当前用户全部任务。2026-08-08 真实接口验收覆盖请假同意/撤回取消、报销驳回取消/同意付款、采购同意执行收货；所有流程进入历史结束态且运行中任务归零。
