...
当前 1.0.0 不提供独立的流程模型草稿、运行时跳转/加签或历史查询 Controller。流程设计器导出的 BPMN XML 直接通过 `ProcessDefinitionController` 部署；流程实例状态、变量、图形和任务轨迹只通过已发布的流程实例与任务接口访问。后续扩展这些能力时必须新增明确的 Service 端口、权限、`@Audit` 和 `version = "1.0.0"` API contract tests，不能恢复空壳 Controller。
