...
当前安全认证授权以 `spectra_security` 目标模型为唯一事实源，干净数据库由 Flyway 单一 `V1__init_db.sql` 初始化。所有 API 版本统一为 `1.0.0`；已移除的旧 API、双写逻辑和旧授权入口不提供兼容层。
...
本地 PostgreSQL/Redis 故障注入、cross-assignment、Flyway 单一 V1 基线和 HTTPS 运行时检查均有自动化验证；浏览器、多端并发和完整业务流程使用人工验收入口。CI、备份恢复、生产 CORS/代理、S3 Object Lock 和 break-glass 演练属于 `1.0.0` 发布及生产运维门禁，详见 [[../60-部署运维/50-生产部署说明]]。
