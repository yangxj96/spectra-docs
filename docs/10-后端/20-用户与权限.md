...
本地 PostgreSQL/Redis 故障注入、cross-assignment、Flyway 迁移基线和 HTTPS 运行时检查均有自动化验证；浏览器、并发和完整业务流程使用人工验收入口。CI、备份恢复、生产 CORS/代理、S3 Object Lock 和 break-glass 演练属于 `1.0.0` 发布及生产运维门禁，详见 [[../60-部署运维/50-生产部署说明]]。
...
权限编码采用树形 `MODULE:*` + 动作叶子节点 `MODULE:QUERY/INSERT/UPDATE/DELETE`。权限判断使用叶子节点，父节点仅用于管理树展示。已完成的超级运维契约是：`security.administrators` 配置的有效 `ROLE_DEV_OPS` 无需维护角色权限关系，即可在后端权限判断中隐式获得 `*`，Token 权限上下文返回 `*`，`GET /menu/current` 返回全部有效菜单；前端 Store、按钮指令和菜单树统一消费这一通配符语义。本系统是单租户、非 SaaS，组织层级只用于同一系统实例内的数据范围控制，不提供租户切换或租户隔离；Root 的 GLOBAL 数据范围、敏感字段脱敏、MFA 和审计仍由独立安全校验负责。
