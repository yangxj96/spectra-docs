...
权限编码采用树形 `MODULE:*` + 动作叶子节点 `MODULE:QUERY/INSERT/UPDATE/DELETE`。权限判断使用叶子节点，父节点仅用于管理树展示；运维角色通过 `*` 获得全量权限。

| 角色 | 权限边界 |
|---|---|
| `ROLE_DEV_OPS` 运维管理员 | `*` 全权限；可执行系统配置、密钥、监控、密码重置、文件信息、权限树和消息发送等运维专属接口 |
...
角色菜单树按权限边界同步：`ROLE_DEV_OPS` 显示完整导航；`ROLE_ADMIN_SYSTEM` 显示系统管理、OA、工作流和示例菜单，但隐藏监控、系统配置、文件信息等运维菜单；`ROLE_USER` 显示首页、OA 业务和示例菜单；`ROLE_AUDIT` 显示首页及审计所需的系统/OA/工作流查询菜单。关联表只保存 `MENU` 叶子节点，当前用户菜单接口自动补齐父级目录。
