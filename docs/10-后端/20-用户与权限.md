...
用户批量导入是独立的 Preview/Apply 任务：Preview 只保存固定模板行、文件摘要和校验结果，Apply 逐行复用用户创建及 RoleAssignment Preview/Apply。任务使用操作人 + 幂等键、规范化请求摘要、授权方案版本摘要和一次性短时 token 防止重复应用；后端不接受内部 Role、Assignment、Permission 或 Scope UUID。
