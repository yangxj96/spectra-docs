...
只有 `ACTIVE` 用户可以创建新的认证 Session。`LOCKED` 是安全策略临时锁定，`DISABLED` 是管理员停用，`DEPARTED` 是离职终态；重新入职不自动恢复历史授权。用户管理页面的新增/编辑通过用户开通提交接口同时保存资料、多角色 RoleAssignment 以及被移除的授权实例，任一环节失败都会回滚本次事务。
