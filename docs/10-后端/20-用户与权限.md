...
敏感接口统一使用 `hasRole('ROLE_DEV_OPS')`：服务监控、系统配置、RSA 密钥读写、用户密码重置、权限树增删改、文件信息删除、内部消息发送、区域基础数据写入，以及 Web 加密 nonce 的定向/全局失效。普通接口按模块动作权限校验，个人资料/菜单当前用户等自服务接口仅要求已认证。
...
| `session:revoke` | 复用既有会话撤销能力，按用户/客户端或用户全部撤销 Session |
