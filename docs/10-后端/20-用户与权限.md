...
`AuthorizationProfile` 是可复用的授权方案模板，不是运行时授权对象。方案使用稳定的 Role/Permission 编码、Role version 和部门业务编码保存配置；应用到用户时仍必须重新通过 RoleAssignment Preview/Apply、目标用户安全版本和 Grant Boundary 校验。方案版本变化不会修改已经生成的 RoleAssignment，停用方案只阻止后续使用。
