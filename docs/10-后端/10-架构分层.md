...
| `AuthenticationWebUtils` | Cookie、CSRF 和认证响应的 HTTP 技术适配；Core 的认证和上传子域只通过 framework 使用 |
| `framework.configure` + `framework.security` | Spring Security 配置入口、Token 过滤器、Session/Token/验证码/初始化令牌 Redis 适配和安全 Redis fail-closed 执行器 |
