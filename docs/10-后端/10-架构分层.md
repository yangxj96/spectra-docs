...
| Starter | 说明 |
|---|---|
| `spectra-security-spring-boot-starter` | Spring Security 自动配置、Redis Session/Token 和安全适配端口 |

统一 `@Audit` 技术切面和自动配置位于必选 `spectra-core`；不再保留独立的 `spectra-log-base` 或 `spectra-log-spring-boot-starter`。
...
`spectra-modules` 和 `spectra-starter` 只是 Maven 聚合层，不是业务运行时层；业务依赖必须落在实际叶子模块的 POM 中，不能为了“方便引用”重复添加聚合层依赖。`core` 不依赖 OA、Workflow、Notification、Upload 或未来 ERP 的业务实现，未来 ERP 由 launch 作为新的可选模块装配。
