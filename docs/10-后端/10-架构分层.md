...
被所有模块依赖的公共层，不依赖任何业务模块。纯基础能力位于 `common.foundation`，集中安全算法位于 `common.security.crypto`；本层仍保留少量历史 MyBatis/Web 技术桥接代码，已登记为后续 B01 整理项。本层不得新增业务反向依赖。
...
| `framework.persistence` | `configuration`、`mybatis`、`scope`；MyBatis-Plus、数据权限组件和持久化拦截器 |
