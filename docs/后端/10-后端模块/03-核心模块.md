...
`spectra-core` 是 Spectra 后端平台的必选业务核心，提供系统运行所需的基础能力与核心编排。Core 不承载 Web、Redis、MyBatis 等纯技术实现，也不依赖 OA、Workflow 或其他业务模块的实现；统一通知和文件上传作为 Core 子域随核心能力提供，跨域调用继续通过 common/framework 的稳定端口接入。
...
- `AuditRecord` 是普通操作、安全操作和认证授权事件共用的不可变模型；`CoreAuditService` 对两种分类执行相同脱敏后，通过唯一的 `JdbcAuditEventWriter` 同步写入 `spectra_core.sys_audit_event`。
- 成功审计记录随业务事务提交；业务异常先回滚，随后由 `AuditFailureRecorder` 在 `REQUIRES_NEW` 事务写入带失败码、异常类型和脱敏原因的 `FAILED` 记录。统一表不可用时审计写入失败会阻断依赖该记录的业务操作。
- `spectra_core.sys_audit_event` 按 `occurred_at` 做 RANGE 分区，包含默认分区和 `(event_id, occurred_at)` 复合主键。应用只拥有 SELECT/INSERT 权限；分区创建、轮换和清理由 DBA 管理，不提供应用归档、保留期或迁移入口。
- 认证、授权和用户生命周期的显式安全审计也调用同一个 `AuditService`；不存在独立安全事实 sink、投递队列或 operation-log worker。
...
| AuditRecord（统一审计支撑表） | spectra_core.sys_audit_event | 普通操作与安全操作的统一分区事实表；不映射为 ORM Entity |
...
| AuditLogController | `/audit/**` | 按分类、操作人 ID/姓名及稳定事件类型/操作说明查询、详情和导出统一审计日志；分页/详情要求 `audit:read`，导出要求 `audit:export` |
...
- [[04-用户与权限]] — 用户权限详细设计
- [[06-系统管理]] — 系统管理详细设计
- [[../20-数据模型/01-实体清单]] — 实体字段说明
- [[16-API总览]] — API 端点总览
- [[11-统一通知模块]] — Core 内置通知子域
