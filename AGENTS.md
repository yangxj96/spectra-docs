...
本文件只保存 Spectra 工作区的全局项目约束。通用 Windows、PowerShell、文件编辑和安全规则由全局 Agent 指令提供；后端、Web 和插件规则由最近的子目录 `AGENTS.md` 提供。
...
领域笔记按目标路由读取：后端用户/权限、系统管理、OA、上传、工作流、基础设施、API 和数据库分别对应 `docs/10-后端/`、`docs/30-数据模型/` 中的相关文件；Web 和插件分别对应 `docs/20-前端/` 中的相关文件。不要读取无关领域文档。
