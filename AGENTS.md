...
`spectra-ui` 通过 `file:` 引用本地 `logicflow-flowable`（`file:../logicflow-flowable`），修改插件代码后需在 `logicflow-flowable/` 执行 `pnpm run build` 或 `pnpm run dev`（监听模式）。
...
# logicflow-flowable（在 logicflow-flowable/ 下执行）
pnpm run build                            # 构建插件
