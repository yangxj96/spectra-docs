...
| 场景 | 使用的 TypeHandler | 说明 |
|---|---|---|
| PostgreSQL `jsonb` 列 | `PgJsonbTypeHandler` | 项目自定义，使用 PGobject |
| MySQL `json` 列 | `Jackson3TypeHandler` | MyBatis-Plus 内置 |
...
- `PgJsonbTypeHandler` 位于 `spectra-common/.../mybatis/PgJsonbTypeHandler.java`
- 它使用 `PGobject` 显式声明类型为 `jsonb`，确保 JDBC 驱动正确处理类型转换
...
**对比：**
```java
// ❌ 错误：Jackson3TypeHandler 会报 "column is of type jsonb but expression is of type character varying"
@TableField(value = "metadata", typeHandler = Jackson3TypeHandler.class)
private Map<String, Object> metadata;

// ✅ 正确：PgJsonbTypeHandler 使用 PGobject 显式声明 jsonb 类型
@TableField(value = "metadata", typeHandler = PgJsonbTypeHandler.class,
            updateStrategy = FieldStrategy.ALWAYS)
private Map<String, Object> metadata;
```
...
完整的代码模板请查看 `references/examples/` 目录：
...
### 错误 5：PostgreSQL jsonb 列使用 Jackson3TypeHandler

```java
// ❌ 错误：Jackson3TypeHandler 将 Map 序列化为 varchar 字符串，PostgreSQL 拒绝
@TableField(value = "metadata", typeHandler = Jackson3TypeHandler.class)
private Map<String, Object> metadata;

// ✅ 正确：使用 PgJsonbTypeHandler，通过 PGobject 显式声明 jsonb 类型
@TableField(value = "metadata", typeHandler = PgJsonbTypeHandler.class,
            updateStrategy = FieldStrategy.ALWAYS)
private Map<String, Object> metadata;
```

**错误表现：** `column "xxx" is of type jsonb but expression is of type character varying`

**根因：** `Jackson3TypeHandler` 是 MyBatis-Plus 内置的 JSON 处理器，它将 Map 序列化为 JSON 字符串传给 PostgreSQL。但 PostgreSQL 的 JDBC 驱动不会自动将 varchar 转为 jsonb，导致类型不匹配。项目自定义的 `PgJsonbTypeHandler` 使用 `PGobject` 显式声明类型为 `jsonb`，解决了这个问题。

## 详细规则查询

完整规范请查阅以下文档：

- 注释规范：`docs/40-规范/15-后端开发规范.md` 第 1 节
- 包结构：`docs/40-规范/15-后端开发规范.md` 第 2 节
- Controller 规范：`docs/40-规范/15-后端开发规范.md` 第 3 节
- Service 规范：`docs/40-规范/15-后端开发规范.md` 第 4 节
- 异常处理：`docs/40-规范/15-后端开发规范.md` 第 5 节
- 实体规范：`docs/40-规范/15-后端开发规范.md` 第 6 节
- From 对象规范：`docs/40-规范/15-后端开发规范.md` 第 7 节
- VO 对象规范：`docs/40-规范/15-后端开发规范.md` 第 8 节
- Converter 规范：`docs/40-规范/15-后端开发规范.md` 第 9 节
- 方法命名规范：`docs/40-规范/15-后端开发规范.md` 第 10 节
- 日志规范：`docs/40-规范/15-后端开发规范.md` 第 11 节
